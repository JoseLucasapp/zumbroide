...
char *ZUM_HL_extensions[] = { ".zum", NULL };
...
void updateSyntax(erow *row){
    ...
    int isZum = (strcmp(E.syntax->filetype, "zum") == 0);
    ...
    while (i < row->rsize) {
        ...
        if(E.syntax->flags & HL_HIGHLIGHT_NUMBERS){
         ...
         if (isZum) {
            if (c == '<' && i + 1 < row->rsize && row->render[i + 1] == '<') {
                row->hl[i] = HL_OPERATOR;
                row->hl[i + 1] = HL_OPERATOR;
                i += 2;
                prev_sep = 1;
                continue;
            }
...
int editorRowCxToRx(erow *row, int cx){
    ...
    int j;
    for(j = 0; j < cx; j++){
        if(row->chars[j] == '\t') rx += (ZUMBROID_TAB_STOP - 1) - (rx % ZUMBROID_TAB_STOP);
        rx++;
    }
    return rx;
}
...
int editorRowRxToCx(erow *row, int rx){
    ...
    for(cx = 0; cx < row->size;cx++){
        if(row->chars[cx] == '\t')
            cur_rx += (ZUMBROID_TAB_STOP - 1) - (cur_rx % ZUMBROID_TAB_STOP);
        cur_rx++;
...
void editorUpdateRow(erow *row){
    ...
    free(row->render);
    row->render = malloc(row->size+ tabs *(ZUMBROID_TAB_STOP - 1)+ 1);

    int idx = 0;
    for(j = 0; j< row->size; j++){
        if(row->chars[j] == '\t') {
            row->render[idx++] = ' ';
            while(idx % ZUMBROID_TAB_STOP != 0) row->render[idx++] = ' ';
        }else{
...
void delCharRow(erow *row, int at){
...
void appendStringRow(erow *row, char *s, size_t len){
...
void delChar(){
    ...
    erow *row = &E.row[E.cy];
    if (E.cx > 0) {
        delCharRow(row, E.cx - 1);
        E.cx--;
    }else{
        E.cx = E.row[E.cy - 1].size;
        appendStringRow(&E.row[E.cy - 1], row->chars, row->size);
        delRow(E.cy);
        E.cy--;
    }
}
...
void editorProcessKeypress(){
    ...
    switch(c){
        ...
        case CTRL_KEY('h'):
        case DEL_KEY:
            if(c == DEL_KEY) moveScreen(ARROW_RIGHT);
            delChar();
            break;
